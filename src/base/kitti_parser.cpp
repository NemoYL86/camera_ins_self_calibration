...
Eigen::Matrix4d
interpolateTransformations(const Eigen::Matrix4d &left,
                           const Eigen::Matrix4d &right,
                           double t)
{
    Eigen::Matrix4d output = Eigen::Matrix4d::Identity();
    // Linearly interpolate the position between the two.
    output.topRightCorner(3, 1) = left.topRightCorner(3, 1) * (1 - t) + right.topRightCorner(3, 1) * t;

    // slerp the rotation between the two.
    output.topLeftCorner(3, 3) =
            Eigen::Quaterniond(Eigen::Matrix3d(left.topLeftCorner(3, 3))).
                    slerp(t, Eigen::Quaterniond(Eigen::Matrix3d(right.topLeftCorner(3, 3)))).toRotationMatrix();
    return output;
}
    ...
    while (std::getline(import_file, line)) {
        ...
        if (header.compare("R") == 0) {
            ...
            if (parseVectorOfDoubles(data, parsed_doubles)) {
                ...
                // (can swap this, but I guess it's anyway easier to just transpose
                // for these small matrices).
                T_vel_imu_.topLeftCorner(3, 3) = R.transpose();
            }
        } else if (header.compare("T") == 0) {
            // Parse the translation matrix.
            if (parseVectorOfDoubles(data, parsed_doubles)) {
                Eigen::Vector3d T(parsed_doubles.data());
                T_vel_imu_.topRightCorner(3, 1) = T;
            }
        }
    }
    std::cout << "Transform T_vel_imu: " << T_vel_imu_ << std::endl;
    return true;
}
    ...
    while (std::getline(import_file, line)) {
        ...
        if (header.compare(0, 1, "R") == 0) {
            ...
            if (parseVectorOfDoubles(data, parsed_doubles)) {
                ...
                // (can swap this, but I guess it's anyway easier to just transpose
                // for these small matrices).
                camera_calibrations_[index].T_cam0_cam.topLeftCorner(3, 3) = R.transpose();
            }
            continue;
            ...
            if (parseVectorOfDoubles(data, parsed_doubles)) {
                Eigen::Vector3d T(parsed_doubles.data());
                camera_calibrations_[index].T_cam0_cam.topRightCorner(3, 1) = T;
            }
            continue;
        }
    }
                                   ...
                                   Eigen::Matrix4d &pose)
{
    ...
    if (!initial_pose_set_) {
        T_initial_pose_.topRightCorner(3, 1) = transform.topRightCorner(3, 1);
        T_initial_pose_.topLeftCorner(3, 3) = axis_yaw.toRotationMatrix();
...
Eigen::Matrix4d KittiParser::T_camN_vel(int cam_number) const
{
    return camera_calibrations_[cam_number].T_cam0_cam * T_cam0_vel_;
}

Eigen::Matrix4d KittiParser::T_camN_imu(int cam_number) const
{
    return T_camN_vel(cam_number) * T_vel_imu_;
}

Eigen::Matrix4d KittiParser::T_cam0_vel() const
{ return T_cam0_vel_; }

Eigen::Matrix4d KittiParser::T_vel_imu() const
{ return T_vel_imu_; }
                                             ...
                                             Eigen::Matrix4d &pose)
{
    ...
    Eigen::Matrix4d transform_left = Eigen::Matrix4d::Identity();
    Eigen::Matrix4d transform_right = Eigen::Matrix4d::Identity();
